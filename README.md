# 💡Digital Finance Literacy Assistant  
*Capstone Project for IBM SkillsBuild 4-Week Internship on AI & Cloud Technologies*

This repository documents the capstone project completed during the **IBM SkillsBuild 4-Week Internship on AI & Cloud Technologies**.  
The program was a collaborative initiative by the **Edunet Foundation, AICTE, and IBM SkillsBuild**, designed to equip students with practical experience in Artificial Intelligence and Cloud technologies.

---

## 📝 Table of Contents
- Intern Details  
- About the Internship  
- Project: Digital Finance Literacy Assistant  
         - Problem Statement    
         - Solution Overview  
- ⚙️ Technology Stack  
- 🚀 Project Workflow  
- 📊 Results  
- 📁 Repository Contents
- 📜 References  

---

## 👩‍💻 Intern Details
- **Name:** Komal Rathod  
- **Institute:** SIGMA INSTITUTE OF ENGINEERING AND TECHNOLOGY  
- **Duration:** 4 Weeks (15th July 2025 – 7th August 2025)  
- **Program:** IBM SkillsBuild 4-Week Internship on AI & Cloud Technologies  

---

## 📖 About the Internship
This 4-week program focused on delivering hands-on skills in **Artificial Intelligence and Cloud Computing** through structured virtual sessions, mentor guidance, and project-based learning.  

### Internship Roadmap:
- **Week 1:** Orientation, IBM Cloud Registration, Access to LMS Portal.  
- **Week 2:** Data Analytics concepts, Hands-On Labs, Generative AI Project, Deploy static website.
- **Week 3:** Build a chatbot and AI agent model, AI/ML experiments on the cloud.
- **Week 4:** Auto AI Projects and Large Visual Model, Create a Jupyter notebook on IBM cloud.

Successful completion required:
- Completing at least two IBM SkillsBuild courses  
- Active participation in mentor sessions  
- Submission of a working final project & presentation  

This internship was offered **without stipend** but provided valuable industry-ready skills.  

---

## 📌 Project: Digital Finance Literacy Assistant  

### Problem Statement
With the rise of digital payments like UPI, many users lack awareness of safe practices. They often face:  
- Risk of fraud  
- Confusion about loans & interest rates  
- Lack of budgeting and saving guidance  
- Barriers due to language diversity
  
### 💡 Solution Overview
The **Digital Finance Literacy Assistant** is an AI chatbot built with **IBM Watson Assistant** and deployed entirely on **IBM Cloud**.  
It provides real-time guidance, multilingual support, and an interactive quiz to test financial knowledge.  
Features:
- ✅ UPI Safety Guidance  
- ✅ Loan & Interest Information  
- ✅ Budgeting & Saving Advice  
- ✅ Fraud Reporting Awareness  
- ✅ Digital Finance Quiz  

---

## ⚙️ Technology Stack
- **Cloud Platform:** IBM Cloud (Lite Plan)  
- **AI Service:** IBM Watson Assistant  
- **Languages Supported:** English, Hindi (scalable to others)  
- **Frontend:** IBM Watson Web Chat (Preview link)  

---

## 🚀 Project Workflow
1. Created a Watson Assistant instance in IBM Cloud  
2. Designed intents and actions for UPI, Loans, Budgeting, and Fraud Awareness  
3. Configured personalized greetings and context handling  
4. Added a Financial Literacy Quiz feature  
5. Deployed and tested using IBM Cloud’s Web Chat Preview  
6. Documented and prepared presentation as per SkillsBuild requirements  

---

## 📊 Results
- Built a fully functional AI chatbot hosted on **IBM Cloud** and also Provided **multilingual financial guidance** successfully. 
- Quiz interaction tested to enhance user learning.
  <img width="1920" height="903" alt="Screenshot (162)" src="https://github.com/user-attachments/assets/3a7b2d1c-1b87-4986-a4e5-16bcb8de4848" />
  <img width="475" height="706" alt="Screenshot (164)" src="https://github.com/user-attachments/assets/c8332809-9893-40fc-8832-0b98c6cbf234" />    <img width="476" height="704" alt="Screenshot (167)" src="https://github.com/user-attachments/assets/6bf973b6-4f9f-40a6-8dd2-0c0e3e811dfd" />

  <img width="478" height="705" alt="Screenshot (173)" src="https://github.com/user-attachments/assets/734982ae-5bd8-4ee2-9277-39f16d33eafd" />    <img width="474" height="704" alt="Screenshot (171)" src="https://github.com/user-attachments/assets/758c2d14-3920-4539-8bdc-c854afeaec3a" />

  <img width="479" height="705" alt="Screenshot (166)" src="https://github.com/user-attachments/assets/6bcc59a9-9aad-4d69-83ba-73a92515d8e9" />    <img width="476" height="706" alt="Screenshot (172)" src="https://github.com/user-attachments/assets/6097b9f8-b6fe-4b02-9e1c-86190d63edbe" />
  
---

## 📁 Repository Contents
- `README.md` → Project Documentation  
- `screenshots/` → Images of working chatbot  
- `Digital finance Literacy Assistant.pdf` → Final Project Presentation  

---

## 📜 References
- [IBM Watson Assistant Documentation](https://cloud.ibm.com/docs/watson-assistant)  
- [IBM Cloud Free Lite Plan](https://www.ibm.com/cloud/free)  
- [Reserve Bank of India – Digital Payment Security](https://www.rbi.org.in)  
- [National Centre for Financial Education](https://www.ncfe.org.in)  
- [Digital India Programme](https://digitalindia.gov.in)  

---

## 🙏 Acknowledgement
Special thanks to **Edunet Foundation, AICTE, and IBM SkillsBuild** for providing this opportunity to learn and implement real-world AI and Cloud technologies.

---
